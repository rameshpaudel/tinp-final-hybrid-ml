...
def train_model(filepath):
    ...
    rf_model = RandomForestClassifier(n_estimators=80, random_state=48)
    svm_model = SVC(probability=True, random_state=48)
    gbm_model = GradientBoostingClassifier(random_state=48)


    # Create voting classifier
    voting_model = VotingClassifier(
        estimators=[('rf', rf_model), ('svm', svm_model), ('gbm', gbm_model)],
        voting='soft'
    )
